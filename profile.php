...
if (isset($_POST['save_profile'])) {
    ...
    if (!empty($_POST['first_name']) && !empty($_POST['last_name']) && !empty($_POST['email'])) {
        // Ваш код для обновления профиля в базе данных

        // Устанавливаем сообщение об успешном сохранении изменений
        $success_message = '<div class="alert alert-success">Изменения успешно сохранены!</div>';
    } else {
        // Если не все обязательные поля заполнены, отображаем сообщение об ошибке
                        ...
                        <div class="profile-left-menu">
                            <nav class="navbar navbar-expand-lg navbar-light">
                                <ul class="navbar-nav flex-row">
                                    ...
                                    <li class="nav-item">
                                        <a class="nav-link-profile" href="orders_history.php">История заказов</a>
                                    </li>
                                    </li>
                                </ul>
                            </nav>
                ...
                <form action="#" method="POST" onsubmit="setTimeout(function(){ location.reload(); }, 2000);">
                    ...
                    <div class="text-center">
                        <button type="submit" id="saveButton" class="profile-btn btn btn-danger"
                            name="save_profile">Сохранить изменения</button>
                    </div>
                </form>
            </div>
        </div>
